...
import {ArtistSearchParams} from './artist-search-params';
...
export class ArtistService implements Findable<Artist>, Listable<Artist>, Deletable<Artist> {
  ...
  private readonly publishedArtistsIndexName = 'artists';
  private readonly unpublishedArtistsIndexName = 'unpublished_artists';

  private DEFAULT_FIND_ALL_PARAMS: ArtistSearchParams = {
    limit: 5,
    published: true,
    unpublished: false
  };
  ...
  search(query: string = '', params?: ArtistSearchParams): Observable<Artist[]> {
    params = params
    ...
    const publishedArtists$ = params.published
      ? this.algolia.query<Artist>(this.publishedArtistsIndexName, parameters)
      : of([]);

    const unpublishedArtists$ = params.unpublished
      ? this.algolia.query<Artist>(this.unpublishedArtistsIndexName, parameters)
      : of([]);

    return combineLatest(publishedArtists$, unpublishedArtists$)
      .pipe(map(([pub, unpub]) => [...pub, ...unpub]));
  }
  ...
  findAllAndSubscribe(filter: string, params?: ArtistSearchParams): Observable<Artist[]> {
